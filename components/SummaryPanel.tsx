...
export default function SummaryPanel({ transactions }: SummaryPanelProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      {/* Tarjeta de Ingresos */}
      <div className="bg-card p-6 rounded-xl border border-border shadow-lg flex flex-col justify-between">
        ...
        <button 
          ...
          className="flex items-center gap-1 text-xs text-muted hover:text-foreground transition-colors self-start mt-2"
        >
          Ver detalle
          ...
          <div className="overflow-hidden">
            <div className="mt-4 pt-3 border-t border-border/50 space-y-2">
              {Object.entries(incomeByCategory).length > 0 ? (
                ...
                <p className="text-xs text-muted italic">No hay ingresos registrados</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="bg-card p-6 rounded-xl border border-border shadow-lg flex flex-col justify-between">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-sm font-medium text-muted">Total Gastos</p>
            <p className="text-2xl font-bold text-foreground mt-1">
              ${totalExpense.toFixed(2)}
            </p>
          </div>
          <div className="bg-background p-3 rounded-full border border-border">
            <TrendingDown className="text-muted" size={24} />
          </div>
        </div>
          ...
          <div className="overflow-hidden">
            <div className="mt-4 pt-3 border-t border-border/50 space-y-2">
              {Object.entries(expensesByCategory).length > 0 ? (
                ...
                <p className="text-xs text-muted italic">No hay gastos registrados</p>
              )}
            </div>
          </div>
        </div>
      ...
      {/* Tarjeta de Balance */}
      <div className="bg-card p-6 rounded-xl border border-border shadow-lg flex flex-col justify-center">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-muted">Balance Actual</p>
            <p className={`text-2xl font-bold mt-1 ${balance >= 0 ? "text-primary" : "text-foreground"}`}>
              ${balance.toFixed(2)}
            </p>
          </div>
          <div className="bg-background p-3 rounded-full border border-border">
